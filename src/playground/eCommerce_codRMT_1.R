...
# Lendo a base de dados de transações de itens. Este comando de leitura j? traz a
# base em formato de matriz esparsa.
order_lineitems_2_R_4 <- read.transactions("data/raw/order_lineitems_2_R_4.csv",
                                           sep = ",")
...
itemFrequencyPlot(order_lineitems_2_R_4, topN = 15)

# Construindo o modelo de Análise de Cesta de Mercado (Market Basket Analysis) para
# itens.
mba_product <- apriori(order_lineitems_2_R_4, parameter = list(support = 0.0001,
                                                               confidence = 0.10,
                                                               minlen = 2))
...
order_productcat_2_R_4 <- read.transactions("data/raw/order_productcat_2_R_4.csv",
                                            sep = ",")
...
mba_product_cat <- apriori(order_productcat_2_R_4, parameter = list(support = 0.01,
                                                                    confidence = 0.10,
                                                                    minlen = 2))

# Explorando o modelo de Análise de Cesta de Mercado constru?do para categorias
# de produtos.
mba_product_cat
...
# Visualização propriamente dita (BLOCO 1: B?SICA)
